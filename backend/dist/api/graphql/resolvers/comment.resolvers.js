...
exports.commentResolvers = {
    Query: {
         ...
         */
        comments: async (_, { feedbackId, limit = 10, offset = 0 }) => {
            try {
                const comments = await Comment_1.Comment.find({ feedback: feedbackId })
                    .sort({ createdAt: -1 })
                    .skip(offset)
                    .limit(limit)
                    .populate('author')
    ...
    Mutation: {
        ...
        createComment: async (_, { input }, context) => {
            ...
            try {
                ...
                const populatedComment = await Comment_1.Comment.findById(comment._id)
                    .populate('author')
                    .populate('likedBy');
                if (!populatedComment) {
         ...
         */
        updateComment: async (_, { id, message }, context) => {
            if (!context.user) {
                throw new apollo_server_express_1.AuthenticationError('You must be logged in');
            }
            try {
                ...
                if (!comment) {
                    throw new apollo_server_express_1.UserInputError('Comment not found');
                }
                if (comment.author.toString() !== context.user.id) {
                    throw new apollo_server_express_1.AuthenticationError('Not authorized to update this comment');
                }
                const updatedComment = await Comment_1.Comment.findByIdAndUpdate(id, { message }, { new: true })
                    .populate('author')
                    .populate('likedBy');
                if (!updatedComment) {
        ...
        deleteComment: async (_, { id }, context) => {
            ...
            try {
                ...
                if (comment.author.toString() !== context.user.id && context.user.role !== 'admin') {
                    throw new apollo_server_express_1.AuthenticationError('Not authorized to delete this comment');
                }
                await Comment_1.Comment.findByIdAndDelete(id);
                return true;
            }
            catch (error) {
        ...
        likeComment: async (_, { id }, context) => {
            ...
            try {
                ...
                const hasLiked = await Comment_1.Comment.findById(id).then((comment) => comment?.likedBy?.includes(context.user.id) || false);
                if (hasLiked) {
                    throw new apollo_server_express_1.UserInputError('You have already liked this comment');
                }
                const updatedComment = await Comment_1.Comment.findByIdAndUpdate(id, {
                    $inc: { likes: 1 },
                ...
                logger_1.logger.error('Error liking comment:', error);
                throw error;
            }
        },
        /**
        ...
        unlikeComment: async (_, { id }, context) => {
            ...
            try {
                ...
                const hasLiked = await Comment_1.Comment.findById(id).then((comment) => comment?.likedBy?.includes(context.user.id) || false);
                if (!hasLiked) {
                    throw new apollo_server_express_1.UserInputError('You have not liked this comment');
                }
                const updatedComment = await Comment_1.Comment.findByIdAndUpdate(id, {
                    $inc: { likes: -1 },
                ...
                logger_1.logger.error('Error unliking comment:', error);
                throw error;
            }
        },
    },
    ...
    Comment: {
        ...
        feedback: async (parent) => {
            const feedback = await Feedback_1.Feedback.findById(parent.feedback);
            return feedback
                ? {
                    ...feedback.toObject(),
        ...
        likesCount: async (parent) => {
            return await Comment_1.Comment.findById(parent._id).then((comment) => comment?.likedBy?.length || 0);
        },
        /**
        ...
        hasLiked: async (parent, _, context) => {
            if (!context.user)
                return null;
            return await Comment_1.Comment.findById(parent._id).then((comment) => comment?.likedBy?.includes(context.user.id) || false);
        },
    },
};
